...
DAG_graph = "U1 -> X1, U1 -> X2,N1 -> X1, X1 -> X2,N2 -> X2, X2 -> Y,N3 -> Y"
unobserved_variables = "U1, N1, N2, N3"
ate = "X2,Y"
data = "id_data.csv"
min_sample = 1000
max_sample = 50000
...
def draw_graph(DAG_graph,
    ...
    x_axis_infeasible = []
    y_axis_infeasible = [0] * int(max_sample/step_sample)
    ...
    for i in range(0, int(max_sample/step_sample)):
        ...
        x_axis_infeasible.append(str(num_samples))
        ...
        for j in range(0, data_points_per_num_samples):
            ...
            dat = raw_data.loc[:, ['X1', 'X2', 'Y']]
            dat = pd.DataFrame(sample_size.groupby(['X1', 'X2', 'Y']).value_counts().reset_index())
            ...
            try:
                ...
                y_axis_infeasible[i] += 1


    fig, (scatter, bar) = plt.subplots(nrows=2, ncols=1, figsize=(12,7))
    ...
    bar.bar(x_axis_infeasible, y_axis_infeasible, color="green")
    bar.set_xlabel("Number of Samples from Observational Data")
    plt.xticks(rotation="vertical")
    bar.set_ylabel("Number of infeasible Solutions")
