...
class DAG:
    ...
    # TODO clean up and make pull request
    def find_u_linked(self,v):
        """
        find all the Vs linked to a specific v through U variables
        the while look puts all Vs that have been traversed in v_checked
        all Vs that have not been traversed but are linked through Us in v_not_checked
        """
        v_checked = set()
        v_not_checked = set()
        v_not_checked.add(v)

        while len(v_not_checked) > 0:
            vnc_with_new_variables = v_not_checked.copy()

            for i in v_not_checked:
                vnc_with_new_variables = vnc_with_new_variables.union(self.get_Vs_linked_through_u(i))
                v_checked.add(i)
            v_not_checked = vnc_with_new_variables.difference(v_not_checked.union(v_checked))

        return v_checked

    def get_Vs_linked_through_u(self, v):
        '''
        get the Vs that are directly linked to a specific U (edge length 1)
        '''
        pa = self.find_parents(v)
        ch = self.find_children(v)
        u_family = (set.union(pa, ch)).intersection(self.U)
        v_family = set()

        for i in u_family:
            # getting all v's linked to the u
            parents = self.find_parents(i)
            children = self.find_children(i)
            pa_ch_v = parents.union(children)
            pa_ch_v = pa_ch_v.intersection(self.V)
            v_family = v_family.union(pa_ch_v)
        return v_family
